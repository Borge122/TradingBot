...
def medium_extractor(alpha0, cellstates):
    ...
    alpha1 = tf.nn.sigmoid(fcl(alpha0, [20, 64, 64], "a-FCL-1a"))
    alpha1, upper_cell1 = dense_lstm(alpha1, [128, 128, 64], "a-LSTM-1b", cellstate_1)

    alpha2 = tf.nn.sigmoid(fcl(alpha1, [64, 128, 128],"a-FCL-2a"))
    alpha2, upper_cell2 = dense_lstm(alpha2, [256, 256, 128], "a-LSTM-2b", cellstate_2)

    alpha3 = tf.nn.sigmoid(fcl(alpha2, [128, 256, 256],"a-FCL-3a"))
    ...
    attention = tf.nn.sigmoid(fcl(tf.concat([beta0, beta1, beta2, beta3], axis=1), [512, 256, 4], "Attention"))
    ...
    alpha4 = tf.nn.sigmoid(fcl(temporary, [512, 256, 128], "Attention2"))
    alpha5, upper_cell4 = dense_lstm(alpha4, [128, 512, 512], "a-LSTM-4b", cellstate_4)
    output = tf.nn.sigmoid(fcl(alpha5, [512, 256, 256, 20], "FCL-Final"))
    ...
    with tf.Session() as sess:
        ...
        for i in range(dataset.shape[1]-1):
            ...
            if not i % 10 == 0:
                ...
                accuracy_list.append(np.mean(np.abs(np.subtract(classifications, dataset[:, i + 1]))))
                print(f"Epoch: {i} - Cost {cost:5.4f} - Accuracy Sign Coefficient {accuracy_list[-1]:2.1f}% - Attention {np.round(attention*100,1)}")
                plt.clf()
